...
parser.add_argument(
    "--motion_file",
    type=str,
    default="sample_data/amass_isaac_standing_upright_slim.pkl",
    help="Path to motion file to load.",
)
parser.add_argument(
    "--policy_path",
    type=str,
    default="output/HumanoidIm/phc_3/Humanoid.pth",
...
parser.add_argument(
    ...
    default="smpl",
    choices=["smpl", "smplx"],
    help="Type of humanoid model to use.",
)
parser.add_argument(
    "--num_motions",
    type=int,
    default=10,
    help="Number of motions to load from motion library.",
)
...
from phc.assets.smpl_config import (
    SMPL_Upright_CFG,
    SMPL_CFG,
    SMPLX_Upright_CFG,
    SMPLX_CFG,
...
from phc.learning.network_loader import (
    load_z_encoder,
    load_z_decoder,
    load_pnn,
    load_mcp_mlp,
)
from phc.utils.isaacgym_humanoid_funcs import (
...
class SMPLEnvCfg(DirectRLEnvCfg):
    ...
    smpl_robot: ArticulationCfg = SMPL_Upright_CFG.replace(
        prim_path="/World/envs/env_.*/Robot"
    )
    # smpl_robot: ArticulationCfg = SMPLX_CFG.replace(prim_path="/World/envs/env_.*/Robot")

    # scene
    scene: InteractiveSceneCfg = SMPLSceneCfg(
        num_envs=args_cli.num_envs, env_spacing=20.0, replicate_physics=True
    )
...
class SMPLEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: SMPLEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        SMPL_NAMES = (
            SMPLH_MUJOCO_NAMES if self.humanoid_type == "smplx" else SMPL_MUJOCO_NAMES
        )
        self.gym_joint_names = gym_joint_names = [
            f"{j}_{axis}" for j in SMPL_NAMES[1:] for axis in ["x", "y", "z"]
        ]
    ...
    def _load_motion(self, motion_file):
        ...
        self._motion_id, self._motion_time = (
            torch.arange(self.num_envs).to(self.device).long(),
            torch.zeros(self.num_envs).to(self.device).float(),
        )
    ...
    def _reset_idx(self, env_ids: Sequence[int]):
        ...
        init_root_state = torch.cat(
            [ref_root_pos, xyzw_to_wxyz(ref_root_rot), ref_root_vel, ref_root_ang_vel],
            dim=-1,
        )
        self.robot.write_root_state_to_sim(init_root_state, env_ids)
        self.robot.write_joint_state_to_sim(
            init_joint_pos, init_joint_vel, None, env_ids
        )
...
def main():
    ...
    pnn = load_pnn(
        check_points[0], num_prim=3, has_lateral=False, activation="silu", device=device
    )
    running_mean, running_var = (
        check_points[-1]["running_mean_std"]["running_mean"],
        check_points[-1]["running_mean_std"]["running_var"],
    )
    ...
    while True:
        ...
        full_obs = (full_obs - running_mean.float()) / torch.sqrt(
            running_var.float() + 1e-05
        )
